...
let MongoDB = exports.MongoDB = function(options) {
  ...
  function setupDatabaseAndEmptyQueue(db, client) {
    if (client) {      
  ...
  function connectToDatabase(logger) {
    logger.pendingConnection = true;
    return mongodb.MongoClient.connect(logger.db, logger.options
    ).then(client=>setupDatabaseAndEmptyQueue(client.db(), client), err=>{
  ...
  if ('string' === typeof this.db) {    
    ...
    //mongoose conection has no db() function, this branch will hit when passing in a mongoose conection immediately
    this.db.then(client => setupDatabaseAndEmptyQueue(typeof client.db === 'function' ? client.db() : client.db), err=>console.error(
...
MongoDB.prototype.close = function() {
  if (this.pendingConnection) {
    //connection not finished yet, but close is called
    this.closed = true;
  } 
  ...
  if (client) {
    //only close those created by this instance
    client.close();
  }
};
